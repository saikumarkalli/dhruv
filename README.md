...
[![Releases](https://img.shields.io/badge/releases-GitHub-blue.svg?style=flat-square)](../../releases)
...
> - [`platform/DESIGN-SYSTEM.md`](platform/DESIGN-SYSTEM.md) — binding design contract for all UI
...
Every component versions independently — a web-only release does not move the Android version
(see [`platform/VERSIONING.md`](platform/VERSIONING.md)). The table below is generated from
[`platform/versions.json`](platform/versions.json); do not edit it by hand.

<!-- version:start -->
<!-- GENERATED by scripts/ci/sync_versions.py — do not edit by hand. -->
| Component | Version |
| --- | --- |
| `apps.finance` | `2.0.4` |
| `web.finance` | `0.1.0` |
...
- **Active app:** Finance — `applicationId = com.dhruv.finance` (`versionCode` tracked in
  `gradle.properties`, CI-owned).
...
- **Distribution:** signed release APK attached to a GitHub Release on every merge to `main`
  (Play Store / AAB deferred — [ADR-0008](platform/DECISIONS.md), [ADR-0032](platform/DECISIONS.md)).
- **Branches:** `develop` is the default branch and is **DEV** (`dhruv-dev`, Vercel Preview, debug
  APK). `main` is **PROD** (`dhruv-prod`, Vercel Production, signed APK + GitHub Release). Both are
  **PR-only** — see [CONTRIBUTING.md](CONTRIBUTING.md#branching). ([ADR-0032](platform/DECISIONS.md)
  supersedes ADR-0009's "`main` is Play-only" rule.)
