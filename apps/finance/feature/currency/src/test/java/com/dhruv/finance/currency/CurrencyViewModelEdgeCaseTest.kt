...
class CurrencyViewModelEdgeCaseTest {
    ...
    private class FakeRepo(
        ...
        override suspend fun getAllRates(): List<CurrencyRateEntity> =
            if (cachedRates.isNotEmpty()) cachedRates
            else rates.map { (code, rate) -> CurrencyRateEntity(currencyCode = code, rate = rate, timestamp = timestamp ?: 0L) }
    ...
    @Test
    fun sameCurrencyIdentity() = runTest(dispatcher) {
    ...
    // ── Zero input ──
    @Test
    fun zeroInput() = runTest(dispatcher) {
        val vm = vm(FakeRepo(mapOf("USD" to 1.0, "EUR" to 0.85)))
        advanceUntilIdle()
        vm.setCurrencyFrom("USD")
    ...
    @Test
    fun networkFailureWithCacheIsOfflineSuccess() = runTest(dispatcher) {
        val cached = listOf(
    ...
    // ── Empty string input ──
    @Test
    fun emptyStringInput() = runTest(dispatcher) {
        val vm = vm(FakeRepo(mapOf("USD" to 1.0, "EUR" to 0.85)))
        advanceUntilIdle()
        vm.setCurrencyInput("")
    ...
    @Test
    fun decimalInput() = runTest(dispatcher) {
    ...
    @Test
    fun crossRatePrecision() = runTest(dispatcher) {
    ...
    @Test
    fun initialStatusIsLoading() = runTest(dispatcher) {
