...
class CalculatorInputRulesTest {
    ...
    @Test fun divisionByZeroKeepsDivZeroMessage() =
        assertEquals("Error: ÷0", arithmeticErrorMessage(ArithmeticException("Division by zero")))
    ...
    @Test fun arcsinDomainIsNotDivZero() =
        assertEquals("Error: domain", arithmeticErrorMessage(ArithmeticException("Arcsin domain violation")))

    @Test fun arccosDomainIsNotDivZero() =
        assertEquals("Error: domain", arithmeticErrorMessage(ArithmeticException("Arccos domain violation")))

    @Test fun unknownArithmeticErrorFallsBackToGenericError() =
        assertEquals("Error", arithmeticErrorMessage(ArithmeticException(null)))

    // ── Bug 2: a trailing `%` must survive a following operator ───────────────

    @Test fun operatorAfterPercentKeepsPercent() =
        assertEquals("50%+", appendOperator("50%", "+"))

    @Test fun minusAfterPercentKeepsPercent() =
        assertEquals("50%-", appendOperator("50%", "-"))

    @Test fun multiplyAfterPercentKeepsPercent() =
        assertEquals("200%×", appendOperator("200%", "×"))
    ...
    @Test fun blankMessageFallsBackToError() =
        assertEquals("Error", arithmeticErrorMessage(ArithmeticException("")))

    @Test fun unrelatedMessageFallsBackToError() =
        assertEquals("Error", arithmeticErrorMessage(ArithmeticException("overflow")))
}
