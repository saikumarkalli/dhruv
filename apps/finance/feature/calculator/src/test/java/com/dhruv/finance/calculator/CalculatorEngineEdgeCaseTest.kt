...
class CalculatorEngineEdgeCaseTest {
    ...
    fun extraClosingParenThrows() { eval("2 + 3)") }

    @Test(expected = RuntimeException::class)
    fun emptyParensThrows() { eval("()") }
    ...
    fun asinAbove1Throws() { evalD("asin(1.0000001)") }

    @Test(expected = ArithmeticException::class)
    fun asinBelowN1Throws() { evalD("asin(-1.0000001)") }

    @Test(expected = ArithmeticException::class)
    fun acosAbove1Throws() { evalD("acos(1.5)") }

    @Test(expected = ArithmeticException::class)
    fun acosBelowN1Throws() { evalD("acos(-1.5)") }
    ...
    @Test(expected = ArithmeticException::class)
    fun log0Throws() { eval("log(0)") }

    @Test(expected = ArithmeticException::class)
    fun logNegThrows() { eval("log(-5)") }

    @Test(expected = ArithmeticException::class)
    fun ln0Throws() { eval("ln(0)") }

    @Test(expected = ArithmeticException::class)
    fun lnNegThrows() { eval("ln(-1)") }
    ...
    @Test(expected = ArithmeticException::class)
    fun sqrtNegThrows() { eval("sqrt(-1)") }

    // ── Division By Zero ──────────────────────────────────────────────────────

    @Test(expected = ArithmeticException::class)
    fun divByZeroInt() { eval("10 / 0") }

    @Test(expected = ArithmeticException::class)
    fun divByZeroFloat() { eval("3.5 / 0.0") }

    @Test(expected = ArithmeticException::class)
    fun divByZeroExpr() { eval("(2 + 3) / (5 - 5)") }

    // ── Unknown / Malformed Input ─────────────────────────────────────────────

    @Test(expected = RuntimeException::class)
    fun unknownFunctionThrows() { eval("xyz(5)") }

    @Test(expected = RuntimeException::class)
    fun unexpectedCharacterThrows() { eval("2 $ 3") }

    @Test(expected = RuntimeException::class)
    fun multipleDecimalPointsThrows() { eval("2.3.4") }
    ...
    private fun eval(expr: String, deg: Boolean = false): Double =
        CalculatorEngine.evaluate(expr, deg)

    private fun evalD(expr: String): Double =
        CalculatorEngine.evaluate(expr, true)
}
