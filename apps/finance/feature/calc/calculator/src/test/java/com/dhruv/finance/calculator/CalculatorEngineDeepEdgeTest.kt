...
class CalculatorEngineDeepEdgeTest {
    ...
    @Test fun quirk_funcThenPowerBindsToArgument() = assertEquals(sin(9.0), ev("sin(3)^2"), D)

    /** `sin(3)!` is parsed as `sin(3!)` for the same reason. */
    @Test fun quirk_funcThenFactorialBindsToArgument() = assertEquals(sin(Math.toRadians(6.0)), ev("sin(3)!", true), D)

    /** `%` is percentage, not modulo: `10 % 3` → (10/100) implicitly times 3. */
    @Test fun quirk_percentIsNotModulo() = assertEquals(0.3, ev("10%3"), D)

    /** Factorial-then-power is unsupported: `^` is only consumed before `!`, so `3!^2` won't parse. */
    @Test(expected = RuntimeException::class)
    fun quirk_factorialThenPowerThrows() {
        ev("3!^2")
    }
