...
`GoalsScreen` renders progress rings; the binding component inventory (`platform/DESIGN-SYSTEM.md`) has
...
standard (`platform/DESIGN-SYSTEM.md` §5), Canvas conventions matching TrendLineChart.
...
the global design system (`platform/DESIGN-SYSTEM.md`) now is for the UI half.
