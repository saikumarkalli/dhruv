...
> Source of the phase sequence: `apps/finance/docs/superpowers/plans/2026-07-12-master-roadmap-personal-app.md`
> §3/§5. R0 detail: `apps/finance/PRODUCTION_READINESS.md`. R2 detail + the TDD pattern this doc
> replicates: `apps/finance/docs/superpowers/plans/2026-07-04-p1-networth-execution-checklist.md`.
...
Source: `apps/finance/PRODUCTION_READINESS.md` Phase 0–6 (tasks T1–T18). Phase 0 (D1–D3) is a
...
`apps/finance/docs/superpowers/plans/2026-07-04-p1-networth-execution-checklist.md` (steps A→J). **Follow it
...
Bottom-up, same pure-logic-first discipline as the Android side (`web/docs/sdd/04-web-app-sdd.md` §3,
...
8. **Design-system code cannot be shared** (Kotlin Compose vs TS/CSS — two by-hand
   ...
   The DhruvNext 4-tab-vs-3-tab nav question this note originally flagged as unresolved was
   resolved (ADR-0024, then superseded to 5 tabs by ADR-0027) — but the underlying risk pattern
   still applies to *future* nav-shape decisions: resolve the ADR before building web navigation
   in parallel, or risk building the wrong nav shape twice.
