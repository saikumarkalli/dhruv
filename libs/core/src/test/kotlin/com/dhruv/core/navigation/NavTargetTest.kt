...
class NavTargetTest {
    private val allTabs = listOf(TabKey.HOME, TabKey.CALC, TabKey.PLAN, TabKey.INSIGHTS)
    ...
    fun `pageIndexFor finds the tab by key, not fixed position`() {
        ...
        assertEquals(2, allTabs.pageIndexFor(NavTarget.SelectTab(TabKey.PLAN)))
        assertEquals(3, allTabs.pageIndexFor(NavTarget.SelectTab(TabKey.INSIGHTS)))
    }

    @Test
    fun `pageIndexFor falls back to the first visible tab when the target tab is hidden`() {
        val insightsHidden = listOf(TabKey.HOME, TabKey.CALC, TabKey.PLAN)
    ...
    fun `pageIndexFor tracks a reordered tab list by key`() {
        val reordered = listOf(TabKey.CALC, TabKey.HOME, TabKey.PLAN, TabKey.INSIGHTS)
